...
import { METRICS_DATA, CHART_DATA } from '../constants';
      ...
      <div className="container mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-16">
          {METRICS_DATA.map((metric, index) => (
            <motion.div
              ...
              whileHover={{ y: -10, boxShadow: "0 20px 40px -10px rgba(228, 2, 101, 0.15)" }}
              viewport={{ once: true }}
            ...
            >
              ...
              <div className="relative z-10">
                <div className="flex justify-between items-start mb-4">
                  <h3 className="text-gray-500 font-semibold uppercase tracking-wider text-xs md:text-sm">{metric.label}</h3>
                ...
                <div className="flex items-end gap-3 mb-4">
                  <span className="text-5xl font-black text-gray-900 tracking-tight">{metric.value}</span>
                  {metric.growth && (
        ...
        >
          <div className="flex flex-col md:flex-row justify-between items-center mb-8">
            <h3 className="text-2xl font-bold text-gray-800">MoM Growth (月次成長)</h3>
            <div className="flex gap-6 text-sm mt-4 md:mt-0">
               <div className="flex items-center gap-2">
                 <span className="w-3 h-3 rounded-full bg-secondary" />
                 <span className="text-gray-600 font-medium">商談数 (Meetings)</span>
               </div>
               <div className="flex items-center gap-2">
                 <span className="w-3 h-3 rounded-full bg-primary" />
                 <span className="text-gray-600 font-medium">案件化数 (Opportunities)</span>
               </div>
            </div>
          </div>
